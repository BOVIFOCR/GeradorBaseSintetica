...
def list_input_images(for_anon=False):
    ...
    for input_fpath in fnames:
        ...
        bg_labels_file_exists = Path(str(input_fpath.labels_fpath()).replace('.json', '.bg.json')).exists()
...
path_tiles = path_output_base / 'tile'
...
for p in (
    path_saida, path_back, path_crop, path_mask, path_tiles
):
