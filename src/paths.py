...
def list_input_images(for_anon=False, randomize=False):
    if os.environ.get("DEBUGGING"):
        yield SamplePath("synthesis_input/input/1bc288dc-718c-4c81-9365-48377215880f.jpg")
        return
    fnames = map(SamplePath, Path(path_entrada).iterdir())
    if randomize:
        fnames = list(fnames)
