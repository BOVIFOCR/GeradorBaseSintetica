    ...
    try:
        for it_idx in range(args.num_iters):
            text_2_image.control_mask_gen(
                tipo_doc="RG", json_arq=json_arq, img_id=img_id, angle=0
            )
    except Exception:
        logging.error(
...
mp_lock(
    fnames, process_main, save_callback=None, num_procs=16, out_path=(paths.mplock_out / "synthesize_bgs").as_posix()
)
